...
public class ActionCustomAddResourceToBank : ReGoapAction<string, object> {
    ...
    public override List<ReGoapState<string, object>> GetSettings(GoapActionStackData<string, object> stackData) {
        ...
        foreach (var pair in stackData.goalState.GetValues()) {
            if (pair.Key.StartsWith(Literals.collectedResource)) {
                ...
                foreach (var banksPair in (Dictionary<CustomBank, Vector3>)stackData.currentState.Get("banks")) {
    ...
    public override void Run(IReGoapAction<string, object> previous, IReGoapAction<string, object> next, ReGoapState<string, object> settings, ReGoapState<string, object> goalState, Action<IReGoapAction<string, object>> done, Action<IReGoapAction<string, object>> fail) {
        ...
        var bank = settings.Get("bank") as CustomBank;
        object quantity;
        ...
        if (bank != null && bank.AddResource(resourcesBag, resourceName, (float)quantity)) {
            ...
            StatCounter.AddStat(transform.parent.name, $"Added to bank { resourceName }", (float)quantity);

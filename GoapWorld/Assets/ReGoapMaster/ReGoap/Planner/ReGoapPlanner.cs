    ...
    public class ReGoapPlanner<T, W> : IGoapPlanner<T, W>
    {
        ...
        public IReGoapGoal<T, W> Plan(IReGoapAgent<T, W> agent, IReGoapGoal<T, W> blacklistGoal = null, Queue<ReGoapActionState<T, W>> currentPlan = null, Action<IReGoapGoal<T, W>> callback = null)
        {
            ...
            while (possibleGoals.Count > 0)
            {
                ...
                var leaf = (ReGoapNode<T, W>)astar.Run(ReGoapNode<T, W>.Instantiate(this, goalState, null, null, null), goalState, settings.MaxIterations, settings.PlanningEarlyExit, debugPlan : settings.DebugPlan);
                if (leaf == null)
                {
                    currentGoal = null;
                    continue;
                ...
                var actionList = result.Select(x => x.Action.GetName()).ToList();
                //foreach (var ac in result) {
                //    var acName = ac.Action.GetName();
                //    var acSettings = ((ReGoapState<string, object>)ac.Settings).Get("resourceName");
                //}
                StatCounter.AddPlan(agent.GetName(), currentGoal.GetName(), Time.time.ToString(), actionList);
